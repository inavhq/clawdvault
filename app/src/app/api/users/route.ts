 ...
 * GET /api/users
 *
 * Get users leaderboard or search for users.
 *
 * Query params:
 * - sortBy: 'volume' | 'tokens' | 'fees' (default: 'volume')
 * - limit: number (default: 100, max: 500)
 * - search: string (optional, searches name and wallet)
 *
 * Response:
...
export async function GET(req: NextRequest) {
  try {
    ...
    const { users, total } = await getUsersLeaderboard({ sortBy, limit, page, search });
